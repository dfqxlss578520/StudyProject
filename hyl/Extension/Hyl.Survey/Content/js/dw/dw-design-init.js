...
$(document).ready(function () {
    ...
    setInterval(intervalSaveSurvey, 5000);
    ...
    $("#dwSurveyQuContentAppUl").sortable({
        ...
        stop: function (event, ui) {
            ...
            var startIndex = ui.item.prev().length;
            ui.item.find(".quInputCase input[name=orderbyid]").val(startIndex);
            ui.item.nextAll().each(function (i, item) {
                startIndex += 1;
                $(item).find(".quInputCase input[name=orderbyid]").val(startIndex);
                $(item).find(".quInputCase input[name=saveTag]").val(0);
            });
        }
    });
    ...
    //点击进入编辑状态
    $("#dwCommonEditRoot").unbind().click(function () {
        return false;
    });
    ...
    $("#dwComEditContent").keyup(function () {
        ...
        if (curEditObj != null) {
            var editOffset = $(curEditObj).offset();
            $("#dwCommonEditRoot").show();
            if ($(curEditObj).html().trim() == "") {
                $("#dwCommonEditRoot").offset({ top: editOffset.top - 9, left: editOffset.left });
            } else {
                $("#dwCommonEditRoot").offset({ top: editOffset.top, left: editOffset.left });
            }
        }
    ...
    function saveSurvey(callback) {
        ...
        if (svyNmSaveTag === "0" || svyNoteSaveTag === "0" || svyAttrSaveTag === "0") {
            ...
            survey.id = questionBelongId;
            survey.SurveyName = encodeURIComponent($("#dwSurveyName").html().trim());
 
            if (svyNoteSaveTag === "0") {
                survey.SurveyDetail.SurveyNote = encodeURIComponent($("#dwSurveyNoteEdit").html());
            }
            ////规则属性
            ////收集规则 
...
function bindQuHoverItem() {
    ...
    $(".dwQuDelete").click(function () {
        ...
        if (confirm("确认要删除此题吗？")) {
            ...
            if (quId != "") {
                var url = questiondelete;//ctx + "/design/question!ajaxDelete.action";
                var data = "id=" + quId;
                $.ajax({
                    url: url,
                    data: data,
...
function saveQus(quItemBody, callback) {
    if (quItemBody[0]) {
        var quType = quItemBody.find("input[name='quType']").val();
        if (quType == "RADIO" || quType == "CHECKBOX" || quType == "FILLBLANK" || quType == "SCORE" || quType == "ORDERQU" || quType == "MULTIFILLBLANK" || quType == "PAGETAG" || quType == "PARAGRAPH" || quType == "CHENRADIO" || quType == "CHENCHECKBOX" || quType == "CHENFBK" || quType == "CHENSCORE") {
            saveAll(quItemBody, callback);
        } else {
            callback();
        }
    } else {
...
** 使用同一方法保存，内部区分不同题型
**/
function saveAll(quItemBody, callback) {
    ...
    if (saveTag == 0) {
        ...
        question.dirId = questionBelongId;
        question.quType = quItemBody.find("input[name='quType']").val();
        question.Id = quItemBody.find("input[name='quId']").val();
        question.orderById = quItemBody.find("input[name='orderById']").val();
        question.isRequired = quItemBody.find("input[name='isRequired']").val();
        question.hv = quItemBody.find("input[name='hv']").val();
        question.randOrder = quItemBody.find("input[name='randOrder']").val();
        question.cellCount = quItemBody.find("input[name='cellCount']").val();
        question.contactsAttr = quItemBody.find("input[name='contactsAttr']").val();
        question.contactsField = quItemBody.find("input[name='contactsField']").val();
        question.tag = svTag;
        question.quName = encodeURIComponent(quItemBody.find(".quCoTitleEdit").html());
        ...
        if (question.quType == "RADIO") {
            ...
            $.each(quItemOptions, function (i) {
                ...
                if (quItemSaveTag == 0) {
                    question.QuRadios[realgroupIndex] = new Object();
                    question.QuRadios[realgroupIndex].OptionName = encodeURIComponent(optionValue);
                    question.QuRadios[realgroupIndex].Id = optionId;
                    question.QuRadios[realgroupIndex].isNote = isNote;
                    question.QuRadios[realgroupIndex].checkType = checkType;
                    question.QuRadios[realgroupIndex].isRequiredFill = isRequiredFill;
                    question.QuRadios[realgroupIndex].OrderById = i;
        ...
        else if (question.quType == "CHECKBOX") {
            question.QuCheckboxes = [];
            $.each(quItemOptions, function (i) {
                var optionValue = $(this).find("label.quCoOptionEdit").html();
                var optionId = $(this).find(".quItemInputCase input[name='quItemId']").val();
                var quItemSaveTag = $(this).find(".quItemInputCase input[name='quItemSaveTag']").val();
                ...
                if (quItemSaveTag == 0) {
                    ...
                    question.QuCheckboxes[realgroupIndex] = new Object();
                    question.QuCheckboxes[realgroupIndex].OptionName = encodeURIComponent(optionValue);
                    question.QuCheckboxes[realgroupIndex].Id = optionId;
                    question.QuCheckboxes[realgroupIndex].isNote = isNote;
                    question.QuCheckboxes[realgroupIndex].checkType = checkType;
                    question.QuCheckboxes[realgroupIndex].isRequiredFill = isRequiredFill;
                    question.QuCheckboxes[realgroupIndex].OrderById = i;
                    realgroupIndex += 1;
                }
                //更新 字母 title标记到选项上.
                $(this).addClass("quOption_" + i);
        ...
        else if (question.quType == "FILLBLANK") {
            question.checkType = quItemBody.find("input[name='checkType']").val();
            question.AnswerInputWidth = quItemBody.find("input[name='answerInputWidth']").val();
            question.AnswerInputRow = quItemBody.find("input[name='answerInputRow']").val();
        }
        else if (question.quType == "SCORE") {
            question.paramInt01 = quItemBody.find("input[name='paramInt01']").val();
            question.paramInt02 = quItemBody.find("input[name='paramInt02']").val();
            question.QuScores = [];
            quItemOptions = quItemBody.find(".quCoItem table.quCoItemTable tr td.quOptionEditTd");
            $.each(quItemOptions, function (i) {
                var optionValue = $(this).find("label.quCoOptionEdit").html();
                var optionId = $(this).find(".quItemInputCase input[name='quItemId']").val();
                var quItemSaveTag = $(this).find(".quItemInputCase input[name='quItemSaveTag']").val();
                if (quItemSaveTag == 0) {
                    question.QuScores[realgroupIndex] = new Object();
                    question.QuScores[realgroupIndex].OptionName = encodeURIComponent(optionValue);
                    question.QuScores[realgroupIndex].Id = optionId;
                    question.QuScores[realgroupIndex].OrderById = i;
            ...
            quItemOptions = quItemBody.find(".quCoItem .quOrderByLeft  li.quCoItemUlLi");
            $.each(quItemOptions, function (i) {
                var optionValue = $(this).find("label.quCoOptionEdit").html();
                var optionId = $(this).find(".quItemInputCase input[name='quItemId']").val();
                var quItemSaveTag = $(this).find(".quItemInputCase input[name='quItemSaveTag']").val();
                if (quItemSaveTag == 0) {
                    question.QuOrderbies[realgroupIndex] = new Object();
                    question.QuOrderbies[realgroupIndex].OptionName = encodeURIComponent(optionValue);
                    question.QuOrderbies[realgroupIndex].Id = optionId;
                    question.QuOrderbies[realgroupIndex].OrderById = i;
            ...
            question.paramInt01 = quItemBody.find("input[name='paramInt01']").val();
            question.paramInt02 = quItemBody.find("input[name='paramInt02']").val();
            question.QuMultiFillblanks = [];
            quItemOptions = quItemBody.find(".quCoItem table.mFillblankTable tr td.mFillblankTableEditTd");
            $.each(quItemOptions, function (i) {
                var optionValue = $(this).find("label.quCoOptionEdit").html();
                var optionId = $(this).find(".quItemInputCase input[name='quItemId']").val();
                var quItemSaveTag = $(this).find(".quItemInputCase input[name='quItemSaveTag']").val();
                if (quItemSaveTag == 0) {
                    question.QuMultiFillblanks[realgroupIndex] = new Object();
                    question.QuMultiFillblanks[realgroupIndex].OptionName = encodeURIComponent(optionValue);
                    question.QuMultiFillblanks[realgroupIndex].Id = optionId;
                    question.QuMultiFillblanks[realgroupIndex].OrderById = i;
        ...
        else if (question.quType == "CHENRADIO" || question.quType == "CHENCHECKBOX" || question.quType == "CHENFBK" || question.quType == "CHENSCORE") {
            question.QuChenColumns = [];
            ...
            $.each(quColumnOptions, function (i) {
                ...
                var quItemSaveTag = $(this).find(".quItemInputCase input[name='quItemSaveTag']").val();
                if (quItemSaveTag == 0) {
                    question.QuChenColumns[realgroupIndex] = new Object();
                    question.QuChenColumns[realgroupIndex].OptionName = encodeURIComponent(optionValue);
                    question.QuChenColumns[realgroupIndex].Id = optionId;
                    question.QuChenColumns[realgroupIndex].OrderById = i;
            ...
            $.each(quRowOptions, function (i) {
                ...
                var quItemSaveTag = $(this).find(".quItemInputCase input[name='quItemSaveTag']").val();
                if (quItemSaveTag == 0) {
                    question.QuChenRows[realgroupIndex] = new Object();
                    question.QuChenRows[realgroupIndex].OptionName = encodeURIComponent(optionValue);
                    question.QuChenRows[realgroupIndex].Id = optionId;
                    question.QuChenRows[realgroupIndex].OrderById = i;
        ...
        $.each(quLogicItems, function (i) {
            ...
            if (logicSaveTag == 0) {
                question.QuestionLogics[realgroupIndex] = new Object();
                question.QuestionLogics[realgroupIndex].id = quLogicId;
                question.QuestionLogics[realgroupIndex].QuItemId = cgQuItemId;
                question.QuestionLogics[realgroupIndex].SkipToQuId = skQuId;
                question.QuestionLogics[realgroupIndex].isvalid = visibility;
                question.QuestionLogics[realgroupIndex].logicType = logicType;
                question.QuestionLogics[realgroupIndex].title = itemIndex;
            ...
            success: function (msg) {
                if (msg != "error") {
                    ...
                    if (jsons.quItems) {
                        ...
                        $.each(jsons.quItems, function (i, item) {
                    ...
                    //列选项
                    if (jsons.quColumnItems) {
                        //var quColumnItems = jsons.quColumnItems;
                        $.each(jsons.quColumnItems, function (i, item) {
                    ...
                    if (jsons.quRowItems) {
                        ...
                        $.each(jsons.quRowItems, function (i, item) {
                    ...
                    if (jsons.quLogics) {
                        ...
                        $.each(jsons.quLogics, function (i, item) {
...
function deleteRadioOption() {
    ...
    if (quOptionId != "" && quOptionId != "0") {
        var url = questionradiodelete; // ctx + "/design/qu-radio!ajaxDelete.action";
        var data = "id=" + quOptionId;
        $.ajax({
            url: url,
            data: data,
            type: "post",
...
function deleteCheckboxOption() {
    ...
    var quOptionId = $(optionParent).find("input[name='quItemId']").val();
    if (quOptionId != "" && quOptionId != "0") {
        var url = questioncheckboxdelete;//ctx + "/design/qu-checkbox!ajaxDelete.action";
        var data = "id=" + quOptionId;
        $.ajax({
            url: url,
            data: data,
...
function deleteScoreOption() {
    ...
    var quOptionId = $(optionParent).find("input[name='quItemId']").val();
    if (quOptionId != "" && quOptionId != "0") {
        var url = questionscoredelete;// ctx + "/design/qu-score!ajaxDelete.action";
        var data = "Id=" + quOptionId;
        $.ajax({
            url: url,
            data: data,
...
function deleteOrderquOption() {
    ...
    var quOptionId = $(optionParent).find("input[name='quItemId']").val();
    if (quOptionId != "" && quOptionId != "0") {
        var url = questionOrderbydelete;//ctx + "/design/qu-orderqu!ajaxDelete.action";
        var data = "id=" + quOptionId;
        $.ajax({
            url: url,
            data: data,
...
function deleteMultiFillblankOption() {
    ...
    var quOptionId = $(optionParent).find("input[name='quItemId']").val();
    if (quOptionId != "" && quOptionId != "0") {
        var url = questionMultiFillblankdelete;// ctx + "/design/qu-multi-fillblank!ajaxDelete.action";
        var data = "id=" + quOptionId;
        $.ajax({
            url: url,
            data: data,
    ...
    $.each(quCoChenTableTrs, function (i) {
        if (i == 0) {
            ...
            if (quType == "CHENRADIO") {
                ...
                $(this).append("<td><input type='text' readonly size='1' class='chenfbktxt'></td>");
...
function addChenRowItem(quItemBody, itemText) {
    ...
    var quType = quItemBody.find("input[name='quType']").val();
    var appendTrHtml = "<tr class='quChenRowTr'>";
...
function deleteChenColumnOption() {
    ...
    var quOptionId = $(optionParent).find("input[name='quItemId']").val();
    if (quOptionId != "" && quOptionId != "0") {
        var url = questionDeleteChenColumn;//ctx + "/design/qu-chen!ajaxDeleteColumn.action";
        var data = "id=" + quOptionId;
        $.ajax({
            url: url,
            data: data,
...
function deleteChenRowOption() {
    ...
    var quOptionId = $(optionParent).find("input[name='quItemId']").val();
    if (quOptionId != "" && quOptionId != "0") {
        var url = questionDeleteChenRow;// ctx + "/design/qu-chen!ajaxDeleteRow.action";
        var data = "id=" + quOptionId;
        $.ajax({
            url: url,
            data: data,
